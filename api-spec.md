...
| ステータスコード | 説明 | メッセージ例 |
| :---- | :---- | :---- |
| 400 Bad Request | リクエストが不正（必須パラメータ不足など） | "デバイス名は必須です。" |
| 401 Unauthorized | 認証トークンがない、または無効 | "認証が必要です。" |
| 403 Forbidden | 認証済みだが、リソースへのアクセス権がない | "この操作を行う権限がありません。" |
| 404 Not Found | 指定されたリソースが見つからない | "指定されたデバイスが見つかりません。" |
| 500 Internal Server Error | サーバー内部で予期せぬエラーが発生 | "サーバーエラーが発生しました。" |
...
| パラメータ | 必須 | 値 |
| :---- | :---- | :---- |
| `period` | 任意 | `month` または `year`（省略時は `month`） |
...
| イベント名 | 送信先 | データ例 | 説明 |
| :---- | :---- | :---- | :---- |
| `update:stats` | Web | `{"deviceId": "...", "todayCount": ..., "totalCount": ...}` | 統計データの更新をWebブラウザに通知する |
